...
export default function CowSearchList({ cows: initialCows }: CowSearchListProps) {
  ...
  const applyFiltersAndSearch = () => {
    ...
    if (searchTerm.trim() !== '') {
      ...
      result = result.filter((cow) => {
        return (
          ...
          (cow.adopter_name && cow.adopter_name.toLowerCase().includes(searchTermLower)) ||
    ...
    if (activeHealthFilters.length > 0) {
      result = result.filter(cow => activeHealthFilters.includes(cow.health_status))
    }
    ...
    if (filters.hasAdopter !== null) {
      if (filters.hasAdopter) {
        result = result.filter(cow => cow.adopter_name && cow.adopter_name.trim() !== '')
      } else {
        result = result.filter(cow => !cow.adopter_name || cow.adopter_name.trim() === '')
      }
    }
    ...
    <>
      <div className="mb-6 flex gap-2">
        <Input
          ...
          className="w-full border border-gray-200 focus:ring-2 focus:ring-gray-200"
        />
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent className="w-56">
            ...
            <DropdownMenuCheckboxItem
              ...
              onCheckedChange={(checked: boolean) => handleHealthStatusFilterChange('healthy', checked)}
            >
              Healthy
            </DropdownMenuCheckboxItem>
            <DropdownMenuCheckboxItem
              checked={filters.healthStatus.sick}
              onCheckedChange={(checked: boolean) => handleHealthStatusFilterChange('sick', checked)}
            >
              Sick
            </DropdownMenuCheckboxItem>
            <DropdownMenuCheckboxItem
              checked={filters.healthStatus.under_treatment}
              onCheckedChange={(checked: boolean) => handleHealthStatusFilterChange('under_treatment', checked)}
            >
              Under Treatment
            </DropdownMenuCheckboxItem>
            <DropdownMenuCheckboxItem
              checked={filters.healthStatus.quarantine}
              onCheckedChange={(checked: boolean) => handleHealthStatusFilterChange('quarantine', checked)}
            >
              Quarantine
            </DropdownMenuCheckboxItem>
            ...
            <DropdownMenuCheckboxItem
              ...
              onCheckedChange={(checked: boolean) => handleSourceFilterChange('rescue', checked)}
            >
              Rescue
            </DropdownMenuCheckboxItem>
            <DropdownMenuCheckboxItem
              checked={filters.source.birth}
              onCheckedChange={(checked: boolean) => handleSourceFilterChange('birth', checked)}
            >
              Birth
            </DropdownMenuCheckboxItem>
            <DropdownMenuCheckboxItem
              checked={filters.source.donation}
              onCheckedChange={(checked: boolean) => handleSourceFilterChange('donation', checked)}
            >
              Donation
            </DropdownMenuCheckboxItem>
            <DropdownMenuCheckboxItem
              checked={filters.source.stray}
              onCheckedChange={(checked: boolean) => handleSourceFilterChange('stray', checked)}
            >
              Stray
            </DropdownMenuCheckboxItem>
            <DropdownMenuCheckboxItem
              checked={filters.source.rescued}
              onCheckedChange={(checked: boolean) => handleSourceFilterChange('rescued', checked)}
            >
              Rescued
            </DropdownMenuCheckboxItem>
            <DropdownMenuCheckboxItem
              checked={filters.source.transferred}
              onCheckedChange={(checked: boolean) => handleSourceFilterChange('transferred', checked)}
            >
              Transferred
            </DropdownMenuCheckboxItem>
            ...
            <DropdownMenuCheckboxItem
              ...
              onCheckedChange={(checked: boolean) => handleAdopterFilterChange(checked ? true : null)}
            >
              Has Adopter
            </DropdownMenuCheckboxItem>
            <DropdownMenuCheckboxItem
              checked={filters.hasAdopter === false}
              onCheckedChange={(checked: boolean) => handleAdopterFilterChange(checked ? false : null)}
            >
              No Adopter
            </DropdownMenuCheckboxItem>
      ...
      {filteredCows.length > 0 ? (
        <>
          <Card className="overflow-hidden" style={{border: '1px solid #dfe3ee'}}>
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="text-left" style={{backgroundColor: '#f7f7f7'}}>
                  <th className="p-4 font-medium" style={{color: '#3b5998'}}>Sr. No.</th>
                  <th className="p-4 font-medium" style={{color: '#3b5998'}}>Health Status</th>
                  <th className="p-4 font-medium" style={{color: '#3b5998'}}>Source</th>
                  <th className="p-4 font-medium" style={{color: '#3b5998'}}>Adopter Name</th>
                  <th className="p-4 font-medium" style={{color: '#3b5998'}}>Created At</th>
                </tr>
              </thead>
          ...
          <div className="flex justify-between items-center mt-4">
            <div className="text-sm" style={{color: '#3b5998'}}>
              Showing {startIndex + 1}-{Math.min(endIndex, filteredCows.length)} of {filteredCows.length} cows
            </div>
        ...
        <div className="p-8 text-center rounded-md border border-gray-200" style={{backgroundColor: '#f7f7f7'}}>
          <p style={{color: '#3b5998'}}>No cows match your search criteria.</p>
        </div>
      )}
